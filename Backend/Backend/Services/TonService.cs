...
namespace Backend.Services
{
    public class TonService
    {
        ...
        public TonService(IDbContextFactory<AppDbContext> contextFactory, IHttpClientFactory httpFactory, IConfiguration configuration, ILogger<TonService> logger)
        {
        ...
        public async Task<bool> CheckTranzaction(string txhHash, decimal amount)
        {
            ...
            try
            {
                ...
                foreach (var tx in transactions.EnumerateArray())
                {
                    ...
                    {
                        if (await checkContext.Tranxactions.AnyAsync(t => t.TxhHash == txhHash || t.TxhHash == txHash))
                        {
